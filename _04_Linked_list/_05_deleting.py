...
class SLinkedList:
    ...
    def delete(self, location):
        if self.head is None:
            ...
            if location == 0: # to delete first element
                if self.head == self.tail: # when only one element is present
                    self.head = None
                    self.tail = None
                ...
                if self.head == self.tail:
                    ...
                    node = self.head  # will found the node before that last node
                    while node:
            ...
            else:  # for given fix location
                index = 0
                temp_node = self.head  # a node before the targeted node
                while index < location - 1:
                    temp_node = temp_node.next
                    index += 1
